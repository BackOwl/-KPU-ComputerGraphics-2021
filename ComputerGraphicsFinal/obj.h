...
public:
	...
	Obj(glm::vec3 outvertex, glm::vec3 outnormal, int num) : outvertex{ outvertex }, outnormal{ outnormal }, num{ num } {}
}; 
...
Obj loadObj(const char* filename)
{
	...
	if (objFile == NULL) {
		...
		return Obj(glm::vec3(0.0), glm::vec3(0.0), 0);
	}
	while (1) {
	...
	Obj obj;
	obj.outvertex = outvertex;
	obj.outnormal = outnormal;
	obj.num = outvertex.size();
	return obj;
}


Obj loadObj_normalize_center(const char* filename)
{
	std::vector< unsigned int > vertexIndices, uvIndices, normalIndices;
	std::vector< glm::vec3 > temp_vertices;
	std::vector< glm::vec2 > temp_uvs;
